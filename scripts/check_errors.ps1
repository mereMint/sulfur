...
if (Test-Path "config.json") {
    $configCheck = python -c "import json; json.load(open('config.json')); print('OK')" 2>&1
...
if (Test-Path "db_helpers.py") {
    # Simple check - just verify imports work
    $dbTest = python -c "from db_helpers import init_db_pool; print('OK')" 2>&1
    ...
    Write-Host "  ⚠ Skipping (db_helpers.py not found)" -ForegroundColor Yellow
}
