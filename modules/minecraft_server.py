...
async def create_or_update_server_properties(config: Dict) -> bool:
    """
    Create or update server.properties file from the bot config.
    This ensures config.json settings (max_players, motd, etc.) are applied to server.properties.
    
    Args:
        config: Minecraft configuration dictionary
    ...
    # Map config settings to server.properties keys
    ...
    # Build properties dict from config
    properties = {}
    for config_key, prop_key in config_to_properties.items():
        if config_key in config:
            value = config[config_key]
            # Convert Python types to properties format
            if isinstance(value, bool):
                properties[prop_key] = 'true' if value else 'false'
            else:
                properties[prop_key] = str(value)
    
    # If the file exists, update it; otherwise create a new one
    if os.path.exists(properties_path):
        success = await apply_server_properties(properties)
        if success:
            logger.info(f"Updated server.properties with config settings: {list(properties.keys())}")
        return success
    else:
        # Create new server.properties with defaults and config
        try:
            os.makedirs(MC_SERVER_DIR, exist_ok=True)
            ...
            with open(properties_path, 'w') as f:
                ...
                f.write(f"#Generated by Sulfur Bot\n")
                for key, value in default_properties.items():
                    f.write(f"{key}={value}\n")
            
            logger.info(f"Created server.properties with config settings: {list(properties.keys())}")
            return True
...
async def start_server(config: Dict) -> Tuple[bool, str]:
    ...
    # Create or update server.properties from config
    # This ensures config.json settings (max_players, motd, etc.) are applied
    await create_or_update_server_properties(config)
