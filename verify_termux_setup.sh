...
# Check for MySQL/MariaDB client (prefer mariadb if available on Termux)
if command -v mariadb &> /dev/null; then
    echo -e "${GREEN}[✓]${NC} mariadb client is installed"
elif command -v mysql &> /dev/null; then
    echo -e "${GREEN}[✓]${NC} mysql client is installed"
else
    echo -e "${RED}[✗]${NC} MySQL/MariaDB client is NOT installed"
    ERRORS=$((ERRORS + 1))
fi

# Check for server process names (mysqld or mariadbd)
if pgrep -x "mysqld" > /dev/null 2>&1 || pgrep -x "mariadbd" > /dev/null 2>&1; then
    echo -e "${GREEN}[✓]${NC} MySQL/MariaDB server appears available"
else
    echo -e "${RED}[✗]${NC} MySQL/MariaDB server not found"
    ERRORS=$((ERRORS + 1))
fi
